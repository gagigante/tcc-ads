...
@injectable()
export class UpdateProjectThumbnailUseCase {
  constructor(
  ...
  public async execute({
    ...
    if (project.thumb_url) {
      await this.storageProvider.deleteFile(project.thumb_url);
    }

    const thumbFilename = await this.storageProvider.saveFile(filename);

    project.thumb_url = thumbFilename;
